...
def rag_search(question: str, k: int = 60):
    ...
    # --- UNIVERSAL_PATTERNS: ключевые паттерны и связанные пункты ---
    UNIVERSAL_PATTERNS = [
        # Зарубежное образование, Болашак, PhD, Назарбаев, master
        (r"(болашак|nazarbayev|назарбаев|phd|доктор наук|зарубеж(ом|ный|ном)|европ|usa|uk|университет [а-я]+|master|магистр|докторант|докторантура|поствузовск)", ["32"]),
        # Пенсия, возраст, освобождение, 4 года до пенсии
        (r"(пенси|возраст|пенсион|по возрасту|лет до пенсии|старше|не обязат[её]н|освобожд|четыр(е|ё)х? лет до пенсии|4 года до пенсии)", ["29", "30"]),
        # Декрет, уход за ребёнком, беременность, больничный
        (r"(декрет|беременн|уход за ребен|ребёнк|материнств|отпуск по уход|болезн|нетрудоспособн|стажировк|воинская служб)", ["29", "30"]),
        # Жалоба, апелляция, спор, претензия, комиссия
        (r"(апелляц|жалоб|обжал|претенз|спорн|оспор|комисси|рассмотрен|пересмотр)", ["17", "18", "20"]),
        # Нарушения, санкции, ответственность, аннулировано
        (r"(наруш|акт|ответственн|аннулир|санкц|дисквалификац|фальсификац|недостоверн|лишен|отстран|отказ|понижен|наказан|обнаружен)", ["10", "11", "12", "45", "46", "47", "62"]),
        # Заявление, форма, egov, портал, приложение, документы
        (r"(форма|заявлени|перечень|приложен|портал|egov|электронное правительство|веб-портал|подача|документ|регистрац)", ["13", "15", "16"]),
        # Внеочередная, досрочная, повторная, ускоренная
        (r"(досрочн|внеочередн|повторн|ускорен|немедленн|неотложн|срочн)", ["31", "32"]),
        # Стажер, первичная, первая категория, впервые
        (r"(стажер|стажёр|первичн|первый раз|впервые|опыт|категор|начал|первая|после года|по истечении года|стаж работы)", ["23", "24", "25"]),
        # Присвоение/подтверждение категории, исключения
        (r"(присвоен|подтвержден|исключени|освобожден|отозван|заменен|поручен|утвержден)", ["28", "29", "30"]),
        # Общее: оценка знаний, портфолио, квалификация
        (r"(оценк|озп|портфолио|квалификац|результат|комплексн(ое|ая) аналитическ)", ["10", "12", "24", "61"]),
    ]

    # --- 1. Сборка по паттернам: максимально покрываем универсальные сценарии ---
    matched = []
    for rx, punkt_nums in UNIVERSAL_PATTERNS:
        if re.search(rx, q_lower):
            for num in punkt_nums:
                matched += [p for p in PUNKTS if p["punkt_num"] == num]

    # --- 2. UNIVERSAL_MAP — твоя ручная карта ключевых слов ---
    mapped = []
    ...
    # --- 5. Trigger-based (универсальные важные номера) ---
    trigger_nums = []
    ...
    # --- 6. Собираем всё в rows, избегаем дублирования ---
    rows = []
    seen = set()
    for l in (matched, mapped, semant, regexed, triggers):
        for p in l:
            key = (p["punkt_num"], p.get("subpunkt_num", ""))
            if key not in seen:
                rows.append(p)
    ...
    # --- 7. Добавляем soft_blocks (по твоей логике критических тем) ---
    rows += soft_blocks(question)

    # --- 8. Fallback если всё пусто ---
    if not rows:
        rows = bm25_fallback(question, 10)

    # --- 9. Финальный постпроцессинг, сортировка, удаление хедеров и дубликатов ---
    rows = unique(rows)
