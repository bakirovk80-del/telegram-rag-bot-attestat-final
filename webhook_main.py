...
def build_human_friendly(q, punkts, official_answer):
    # --- Универсальный поиск всех оснований для освобождения/льгот ---
    exception_phrases = []
    for p in punkts:
        txt = p["text"].lower()
        num = f"(п. {p['punkt_num']})" if p.get('punkt_num') else ""
        # Болашак и зарубежные выпускники
        if (("болашақ" in txt or "bolashak" in txt or "зарубеж" in txt or "иностранн" in txt)
            and ("освобожд" in txt or "без прохождения процедуры" in txt or "без аттестации" in txt)):
            exception_phrases.append(
                f"Выпускники зарубежных организаций, входящих в список рекомендованных по программе «Болашақ», освобождаются от аттестации {num}."
            )
        # Ученые степени
        if (("кандидат наук" in txt or "доктор наук" in txt or "phd" in txt)
            ...
            exception_phrases.append(
                f"Педагогам, имеющим степень кандидата/доктора наук или доктора PhD, квалификационная категория присваивается без процедуры аттестации {num}."
            )
        # Пенсионный возраст
        if (("пенси" in txt or "пенсион" in txt or "возраст" in txt)
            and "освобожд" in txt):
            exception_phrases.append(
                f"Педагоги, которым до пенсии по возрасту остается не более четырех лет, освобождаются от аттестации {num}."
            )
        # Инвалидность
        if (("инвалид" in txt or "ограниченн" in txt)
            and "освобожд" in txt):
            exception_phrases.append(
                f"Педагоги с инвалидностью освобождаются от прохождения аттестации {num}."
            )
        ...
        if (("декрет" in txt or "беремен" in txt or "уход за ребен" in txt or "по уходу за ребен" in txt)
            ...
            exception_phrases.append(
                f"Педагоги, находящиеся в декретном отпуске или в отпуске по уходу за ребенком, могут быть освобождены или получить отсрочку от аттестации {num}."
            )
        ...
        if (("воинск" in txt or "военная служб" in txt or "стажировк" in txt or "болезн" in txt or "нетрудоспособн" in txt)
            and ("освобожд" in txt or "отсроч" in txt or "приостан" in txt)):
            exception_phrases.append(
                f"Педагоги, находящиеся на воинской службе, в стажировке, по болезни, могут быть освобождены или получить отсрочку от аттестации {num}."
            )
        ...
        if (("досрочн" in txt or "внеочередн" in txt or "ускорен" in txt or "повторн" in txt)
            ...
            exception_phrases.append(
                f"Досрочное или внеочередное присвоение квалификационной категории допускается по установленной процедуре {num}."
            )
    # Если есть хотя бы одна спец-льгота — объединить их в краткий ответ
    if exception_phrases:
        return " ".join(sorted(set(exception_phrases)))
    # Спец-кейс для платности (можешь оставить свой)
    ...
    # Если ничего не нашли — генерим обычный краткий ответ на основе официального
    prompt = (
        f"Вопрос: {q}\n"
...
def filter_related_punkts(q, all_punkts, cited_nums):
    ...
    """
    q_lower = q.lower()
    # STOP-пункты, которые никогда не нужно выводить в "Связанные"
    STOP_PUNKTS = {"1", "2"}  # Дополняй, если нужно
    ...
    themes = {
        ...
        # Нарушения, ответственность, аннулирование, дисциплина, санкции
        ...
        # Апелляции, жалобы, оспаривание, споры, обжалование, претензии
        "жалоб": r"жалоб|апелляц|обжал|претенз|спорн|оспор|несоглас|пересмотр|рассмотрен|обратиться|претензия|спор",
        # Портал, подача через eGov, электронное правительство, заявление, регистрация
        ...
        # Категория, присвоение, подтверждение, повышение, квалификация
        ...
        # Аттестация, порядок, сроки, этапы, процедура, повторная, внеочередная, ускоренная, неотложная
        ...
        # Документы, приложения, формы, уведомления, протоколы, листы, выписки, приказы, заключения, заявление, справка
        "документ": r"документ|форма|приложен|уведомлени|решени|протокол|приказ|лист|выписка|заключени|акт|заявление|справка",
        # Отсрочка, приостановка, перенос, сохранение, продление, исключение, стажировка, болезни, воинская служба
        ...
        # Комиссия, рассмотрение, назначение, решение, заседание, рассмотрение комиссии, эксперты, экспертная комиссия
        "комисси": r"комисси|рассмотрен|назначен|назначени|решени|заседание|рассмотрение комиссии|члены комиссии|эксперт",
        # Стажировка, испытательный срок, обучение, практика, интернатура, обучение за рубежом
        ...
        # Языки, иностранные языки, экзамены на иностранном
        "язык": r"язык|иностранн|английск|немецк|франц|китайск|турецк|арабск",
    }
    # Собираем все ключевые темы, которые встречаются в вопросе
    ...
    for p in all_punkts:
        pn = p.get("punkt_num", "")
        # Исключаем уже процитированные и стоп-пункты
        if pn in cited_nums or pn in STOP_PUNKTS or not pn or not pn.isdigit():
            continue
        # Совпадение по тематике (по ключевым словам)
