...
def build_human_friendly(q, punkts, official_answer):
    prompt = (
        f"Вопрос: {q}\n"
        "Ответь на этот вопрос очень кратко и строго официальным языком, используя только формулировки из официального ответа. "
        "Не используй разговорные слова вроде «можно», «обычно», «хочет пройти», «есть возможность». "
        "Отвечай в формате: кто, когда и за что платит (если вопрос о платности); перечисли основания и исключения, если есть, отдельным списком. "
        "Ответ не должен содержать дополнительных пояснений или бытовых выражений. "
        "Если вопрос о процедуре — опиши основные этапы в виде короткого списка (1., 2., 3.) только по Правилам. "
        "Не пиши 'по закону' или 'по документу' — только сухие, точные формулировки. "
        "Сохраняй стиль официальных разъяснений для юриста или госслужащего.\n\n"
        f"Официальный ответ:\n{official_answer}\n\n"
        "Твой очень краткий строго-формальный ответ:"
    )
    try:
