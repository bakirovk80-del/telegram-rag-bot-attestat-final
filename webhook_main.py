...
def build_human_friendly(q, punkts, official_answer):
    """
    Формирует краткий формальный ответ, включающий ВСЕ релевантные основания по теме вопроса,
    ориентируясь на ключевые слова в вопросе пользователя, а не только на текст пунктов.
    """
    q_lower = q.lower()
    themes = [
        (
            r"(магистратур[а-я]* за рубежом|зарубежн[а-я]* вуз|иностранн[а-я]* диплом|болашақ|bolashak|nazarbayev|foreign university|foreign master|foreign phd)",
            "Выпускники зарубежных организаций, входящих в список рекомендованных по программе «Болашақ», освобождаются от аттестации (п. 32). В остальных случаях аттестация обязательна (п. 5)."
        ),
        (
            r"(пенси[а-я]*|пенсион[а-я]*|до скольки лет|лет|возраст|старше)",
            "Педагоги, которым до пенсии по возрасту остается не более четырех лет, освобождаются от аттестации (п. 30)."
        ),
        (
            r"(декрет|беремен|уход за ребен|отпуск по уход|нетрудоспособн|болезн|инвалид|ограниченн)",
            "Педагоги, находящиеся в декретном отпуске, отпуске по уходу за ребенком, с инвалидностью или по болезни, могут быть освобождены или получить отсрочку от аттестации (пп. 29-30)."
        ),
        (
            r"(досрочн[а-я]*|внеочередн[а-я]*|ускорен[а-я]*|повторн[а-я]*|срочн[а-я]*|немедлен[а-я]*|раньше срока)",
            "Досрочное или внеочередное присвоение квалификационной категории допускается по установленной процедуре (пп. 63-64)."
        ),
        (
        ...
        (
            r"(степень кандидата|кандидат наук|доктор наук|phd|научная степень)",
            "Педагогам, имеющим степень кандидата/доктора наук или доктора PhD, квалификационная категория присваивается без процедуры аттестации (п. 32)."
        ),
        (
            r"(стаж[её]р|стажировка|стажёры|первичный стаж|испытательный срок)",
            "Педагог-стажёр может подать заявление на присвоение категории «педагог» по истечении одного года работы без прохождения процедуры аттестации (п. 24)."
        ),
        # ...добавляй новые regex/темы ниже по аналогии, если нужно!
    ]
    result = []
    for rx, phrase in themes:
        if re.search(rx, q_lower):
            result.append(phrase)
    if result:
        # Через пробел или точку с запятой — как удобнее
        return " ".join(result)
    # Если не нашли ни одного релевантного основания по ключевым словам, fallback через LLM
    prompt = (
        f"Вопрос: {q}\n"
...
def filter_related_punkts(q, all_punkts, cited_nums):
    """
    Возвращает только реально релевантные связанные пункты по тематике вопроса:
    - не более 10 уникальных
    - пункт считается релевантным, если содержит минимум 2 разных ключевых слова из вопроса
    - fallback: тематика через themes только если совпадений по ключам мало
    """
    q_lower = q.lower()
    ...
    # --- Шаг 1: Сначала ищем по ключевым словам из вопроса (min длина 5 символов) ---
    keywords = set(word for word in re.findall(r"\w+", q_lower) if len(word) > 4)
    related = []
    for p in all_punkts:
        pn = p.get("punkt_num", "")
        txt = p.get("text", "").lower()
        if pn in cited_nums or pn in STOP_PUNKTS or not pn or not pn.isdigit() or not txt:
            continue
        matches = [k for k in keywords if k in txt]
        if len(set(matches)) >= 2:  # Только если хотя бы 2 разных ключевых слова из вопроса есть в тексте пункта!
            related.append(p)
        if len(related) >= 10:
            break

    # --- Шаг 2: Fallback — если пунктов < 3, ищем по тематике (themes) ---
    if len(related) < 3:
