...
UNIVERSAL_MAP = {
    "магистратура за рубежом": {"punkt_num": "32"},
    "закончил магистратуру за границей": {"punkt_num": "32"},
    "иностранная магистратура": {"punkt_num": "32"},
    "болашак": {"punkt_num": "32"},
    "bolashak": {"punkt_num": "32"},
    "nazarbayev university": {"punkt_num": "32"},
    "за пределами рк": {"punkt_num": "5"},
    "за пределами республики казахстан": {"punkt_num": "5"},
    "за границей": {"punkt_num": "5"},
}
...
MISC_MAP = {
    ...
    "квалификационная категория":    {"punkt_num": "32", "subpunkt_num": ""},
    "присвоение категории":          {"punkt_num": "32", "subpunkt_num": ""},

    # Часто спрашивают про льготы — отправляем на общую норму или спец-норму
...
# Быстрая самопроверка (не используется ботом, но поможет при ручном запуске этого файла)
if __name__ == "__main__":
    print(f"Всего триггеров: {len(UNIVERSAL_MAP)}")
    # Пример: покажем, какие пункты подсветятся на вопрос о зарубежной магистратуре
    q = "для тех кто закончил магистратуру зарубежом нужно ли сдавать аттестацию"
    hits = [ (k, UNIVERSAL_MAP[k]) for k in UNIVERSAL_MAP.keys() if k in q ]
